...
from tensorrt_llm.models import LLaMAConfig, LLamaForSequenceClassification
...
def convert_and_save_meta(args, rank):
    mapping = Mapping(world_size=args.tp_size * args.pp_size,
                      ...
                      rank=rank)
    llama = LLamaForSequenceClassification.from_meta_ckpt(
        args.meta_ckpt_dir,
        args.dtype,
...
def convert_and_save_hf(args):
    ...
    if args.smoothquant is not None or args.int8_kv_cache:
        ...
        LLamaForSequenceClassification.quantize(
            args.model_dir,
            args.output_dir,
            dtype=args.dtype,
            mapping=mapping,
        ...
        hf_model = None
        print(f"args.load_by_shard={args.load_by_shard}")
        print(f"has_safetensors(model_dir)={has_safetensors(model_dir)}")
        print(f"quant_config.quant_mode.has_any_quant()={quant_config.quant_mode.has_any_quant()}")
        ...
        def convert_and_save_rank(args, rank):
            mapping = Mapping(world_size=world_size,
                              ...
                              moe_tp_size=args.moe_tp_size,
                              moe_ep_size=args.moe_ep_size)
            llama = LLamaForSequenceClassification.from_hugging_face(
